    ...
    public class PedidoIntegrationHandler : BackgroundService
    {
        ...
        private async Task FinalizarPedido(PedidoPagoIntegrationEvent message)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                var logger = scope.ServiceProvider.GetRequiredService<ILogger<PedidoIntegrationHandler>>();
                logger.LogInformation("Método FinalizarPedido executado com sucesso - pedido finalizado");
            }
        }
    }
}